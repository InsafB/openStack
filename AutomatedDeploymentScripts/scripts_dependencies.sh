...
if [[ ("$serverName" = "ServerI") || ("$serverName" = "ServerS") ]]; then
    ...
    if [[ ("$serverName" = "ServerS") ]]; then
        mysql -u root -pothmane -e "create database open_stack;use open_stack;create table plays(id int(11) primary key,timeplay datetime)"
    fi
fi    
